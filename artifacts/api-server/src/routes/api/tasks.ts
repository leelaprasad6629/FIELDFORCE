...
router.post("/tasks", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const task = await Task.create({ taskId: taskId ?? `TSK-${Date.now()}`, title, category: category ?? "General", assignedTo: assignedTo ?? null, assignedTechnicianId: assignedTechnicianId ?? null, serviceRequestId: serviceRequestId ?? null, status: status ?? "pending", zone, location, priority: priority ?? "medium", eta: eta ? new Date(eta) : null, checklist: checklist ?? [] });
